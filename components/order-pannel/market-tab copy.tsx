...
export function MarketTab({ setSlippageDialogOpen }: MarketTabProps) {
  ...
  const { solBalance, fetchSolBalance } = useSolBalance();
  ...
  const [tokenABalance, tokenBBalance] = useMemo(
    () =>
      isReverse ? [solBalance, tokenBalance?.amount || 0] : [tokenBalance?.amount || 0, solBalance],
            ...
            <div className="flex space-x-2 items-center">
              <div className="flex items-center space-x-1">
                ...
                <span className="text-muted-foreground text-xs">
                  {formatBalance(tokenABalance, 3, inputToken?.decimals)}
                </span>
              </div>
              <Button
                size="xs"
